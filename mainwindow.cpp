    ...
    , ui(new Ui::MainWindow)
{
    ui->setupUi(this);
    StartGame();
    fame.load("C:/Users/Carl/Documents/GitHub/MineSweeper/source.png");
}
...
void MainWindow::TimeOut() {
    if (myGame.GetState() != STATE::INGAME) { timer->stop(); }
    ++this->time;
    update();
}
...
void MainWindow::mouseDoubleClickEvent(QMouseEvent *event) {
    int x = (event->pos().x() - 25) / 20, y = (event->pos().y() - 70) / 20;
    if (event->button() == Qt::LeftButton) {
        if (myGame.DoubleClick(x, y) && firstClick) {
            this->timer->start(1000);
            firstClick = false;
        }
    }
